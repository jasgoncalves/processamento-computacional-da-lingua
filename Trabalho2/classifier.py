...
def import_dataset(path : str, columns : str) -> DataFrame:
    ...
    return read_csv(path, sep = DELIMITER, names = columns, encoding = enc['encoding']) # '\t' for tab delimiter (.tsv)

def ngram_load_data(file_name : str, counts_folder : str) -> DataFrame:
    return\
        import_dataset(f'{ROOT}{counts_folder}/{file_name}{Label.GEOGRAPHY.name}{EXTENSION}', LABEL_COLUMNS),\
        import_dataset(f'{ROOT}{counts_folder}/{file_name}{Label.HISTORY.name}{EXTENSION}', LABEL_COLUMNS),\
        import_dataset(f'{ROOT}{counts_folder}/{file_name}{Label.LITERATURE.name}{EXTENSION}', LABEL_COLUMNS),\
        import_dataset(f'{ROOT}{counts_folder}/{file_name}{Label.MUSIC.name}{EXTENSION}', LABEL_COLUMNS),\
        import_dataset(f'{ROOT}{counts_folder}/{file_name}{Label.SCIENCE.name}{EXTENSION}', LABEL_COLUMNS)
